...
│   ├── test_phase3.py           # 測試：Web UI 結構完整性、設定/MIB API
│   └── test_phase4.py           # 測試：可靠性、部署檔案、community 熱更新
└── resources/mibs.tar.gz        # 內建 MIB 檔案包
```
...
```bash
pip3 install flask pysnmp pysmi --break-system-packages
```
...
```bash
git clone https://github.com/echoapus/simple-nms.git
cd simple-nms
...
也可以從 Web UI 上傳 MIB，成功後會立即嘗試載入到目前執行中的 SNMP resolver。

### 驗證 community 熱更新

在 Web UI 修改 SNMP community 後，可用以下腳本確認不用重啟也能收到 trap：
...
可在 Web UI 使用 **Clear Old Events**，或直接呼叫 cleanup API：
...
若要用 cron 或本機維護指令，也可以使用 `cleanup.py`：
